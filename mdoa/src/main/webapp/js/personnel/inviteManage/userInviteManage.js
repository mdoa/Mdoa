...
$(function(){
	...
	$('#userInviteManage .printIdCardBtn').click(function(){
		...
		$('#userInviteManage .printIdCard img:eq(1)').attr('src',$('#userManage .basic .pic2').attr('src'));
		$('#userInviteManage .printIdCard img:eq(0)').attr('src',$('#userManage .basic .pic3').attr('src'));
	})
	//打印身份证
	...
	$('#userInviteManagedg').datagrid({
		   ...
		   columns:[[
		       {field:"_opera",title:"管理",width:100,resizable:true,align:"center",sortable:true,formatter:function(value,row,index){
		    	   ...
		    	   if(existPermission('admin:personnel:staffRecord:userInviteManage:update'))
		    		   opera += '<span style="float:left;" class="small-button edit" title="修改" onclick="userIntviteUpdateUserInfo('+id+','+idCard+')"></span>'
	    		   if(existPermission('admin:personnel:staffRecord:userInviteManage:select'))
		    		   opera += '<span style="float:left;" class="small-button look" title="查看" onclick="userIntviteLookUserInfo('+id+','+idCard+')"></span>'
	    		   if(existPermission('admin:personnel:staffRecord:userInviteManage:delete'))
		    		   opera += '<span style="float:left;" class="small-button delete" title="删除" onclick="userIntvitedeleteUserInfo('+id+','+userName+')"></span>';
		    	   if(existPermission('admin:personnel:staffRecord:userInviteManage:manage')){
		    		   if(row.inviteFlag == 0){
		    			   opera += '<span style="float:left;" class="small-button govern" title="录用" onclick="userInvite('+id+','+userName+')"></span>';
		    		   }		    		   
		    	   }
		    	   opera += '</div>'
		    	   return opera;
...
/*-------------------点击录用员工-----------------*/
function userInvite(id,userName){
	var flag = false;	
