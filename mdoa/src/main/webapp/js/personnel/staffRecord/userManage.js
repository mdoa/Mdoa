...
$(function(){	
	...
	if(existPermission('admin:personnel:staffRecord:userManage:add')){
		var topHtml = 	'<div style="margin-right:6px;"><input type="button" value="添加" class="button adduser" /></div>'+
						'<div style="margin-right:6px;"><input type="button" value="导出查询结果到Excel" class="button write" /></div>';
	...
	$('#userManage .printIdCardBtn').click(function(){
		...
		$('#userManage .printIdCard img:eq(1)').attr('src',$('#userManage .basic .pic2').attr('src'));
		$('#userManage .printIdCard img:eq(0)').attr('src',$('#userManage .basic .pic3').attr('src'));
	})
	//打印身份证
	$('#userManage .printIdCard .confirm').click(function(){
		$('#userManage .printIdCard').css('display','none');
		$('#userManage .printIdCard .popuparea:eq(0)').jqprint();
	});
	//搜索条 读取身份证
...
//		   url:'../../userInfo/selectUserInfo.do?getMs='+getMS(),
		   ...
		   columns:[[
		       ...
		       {field:"_opera",title:"管理",width:100,resizable:true,align:"center",sortable:true,formatter:function(value,row,index){
		    	   ...
		    	   if(existPermission('admin:personnel:staffRecord:userManage:update'))
		    		   opera += '<span style="float:left;" class="small-button edit" title="修改" onclick="userManageUpdateUserInfo('+id+','+idCard+')"></span>'
	...
	$('#userManage .read-idCard').click(function(){
		...
		var uesrManageReadIdcard = function(){
			...
			var idCardValid = validEnd.substring(0, 4) + '-' + validEnd.substring(4, 6) + '-' +validEnd.substring(6, 8);
					...
					success:function(data){
						...
						if(obj.rows.length > 0){
							if(userId){
								...
								windowControl('该员工为在职员工，无法进行录用<br>'
										+'姓名：'+WidControl.GetName()
										+'<br>身份证号：'+idCard);
							}
						}else{
...
/*-------------------点击修改员工信息-----------------*/
function userManageUpdateUserInfo(id,idcard){
	//必填* 出现
				...
				for(var i = 0 ; i < rows.length ; i++){
					...
					if(inviteFlag == '1' || inviteFlag == '4' || inviteFlag == '5'){	
						if(id == null){
							...
							if(flag == 'update'){
								windowControl('该身份证为在职员工身份证，无法进行修改'+'<br>身份证号：'+idCard);
...
function userManageAddUpdateEvent(){
		...
		$.ajax({
			...
			success:function(data){
				if(data == 400){
					...
					$.ajax({
						data:{operaPanel:"员工管理",operaTab:"修改"},
	...
	$("#userManage .popups .writetoexcel .btnarea input[name=ensure]").click(function(){
		...
		userManageExcel.push({
			name:'jsonString',value:jsonString
		})
		//提交导出excel所需要的参数
		excelplugin.submit({
			type:'post',
			array:userManageExcel,
...
//-----------------员工调动方法和批量调动方法------------------------------
function userTransfer(id,departmentUrl,departmentName,postId,postName,idCard){
