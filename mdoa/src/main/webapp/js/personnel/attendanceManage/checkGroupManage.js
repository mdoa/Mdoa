$(function(){	
	...
	$('#checkGroupManagedg').datagrid({
		...
		onDblClickRow:function(rowIndex,rowData){
			...
			timeWidget('checkGroupManage',groupName,toWorkTime,outWorkTime,groupId);
		},
		columns:[[			
			{
				field:'groupName',
...
//时间控件
function timeWidget(obj,groupName,toWorkTime,outWorkTime,groupId){
	/******************时间控件*************************/
	/*
        ...
        $.ajax({
    		...
    		success:function(data){
    			...
    			for(var i = 0 ; i < dataObj.length ; i++){
    				...
    				for(var j = 0 ; j < $("#"+ obj +" .calendarTable").find("td").length ; j++){
    					if(classDateStr == $("#"+ obj +" .calendarTable").find("td").eq(j).attr('data')){
    						//做个该天不是默认上班时间和下班时间的标记，data属性值为1，则说明该天有班次记录
    						$("#"+ obj +" .calendarTable").find("td").eq(j).attr('aleady',1);
    						$("#"+ obj +" .calendarTable").find("td").eq(j).find('.onDutyTime').val(dataObj[i].onDutyTime);
    						$("#"+ obj +" .calendarTable").find("td").eq(j).find('.offDutyTime').val(dataObj[i].offDutyTime);
    					}
    				}
    			}    			
    ...
    */
    function renderHtml() {
        ...
        titleBox += "<div class='calendarTop'>"+
			            ...
			            "<span style='float:left;line-height:44px;margin-left:20px;'>"+ groupName +"</span>"+
			          "</div></div>";       
          
        $("#"+ obj +" .Calendar").html(titleBox);    // 添加到calendar div中
