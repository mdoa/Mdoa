$(function(){
	...
	/*$('#assetsManage .invitetop .clean').click(function(){
		alert(123);
		$('#assetsManage input[name=assetsName]').val('');
		$('#assetsManage input[name=bedep]').val('');
	});*/
	//条件查询
			...
			if(calMethod==2){
				if(intendWorkGross == ''||intendWorkGross == null){
					...
					var info = {
						...
						defPevWorkGross:defPevWorkGross,
	...
	$.ajax({
		...
		success:function(data){
			...
			if(flag == 2){
				...
				$('#assetsManage .editAssets .defPevWorkGross').val(data.rows[0].defPevWorkGross);
			}else{
				$('#assetsManage .editAssets .normal').show();
				$('#assetsManage .editAssets .workLoad').hide();
