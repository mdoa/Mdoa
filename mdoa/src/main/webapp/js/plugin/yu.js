var F={
	...
	,aimg:function(o){
		...
		F.on(o,'mouseover',function(){
			...
			this.ob.style.top=wz.y-s+'px';
			this.ob.style.left=wz.x+'px';
...
(function(w){
	...
	function div(ob){
		this.cj=function(){
			...
			this.G_d.style.position='absolute';
			this.G_d.style.zIndex=99;
			this.G_d.style.top='0px';
			this.G_d.style.left='0px';
			this.G_d.style.filter='alpha(opacity=0)';
			this.G_d.style.opacity=0;
			this.G_d.style.width=Math.max(d.documentElement.scrollWidth,d.documentElement.clientWidth)+'px';
			this.G_d.style.height=Math.max(d.documentElement.scrollHeight,d.documentElement.clientHeight)+'px';
			this.G_d.style.margin='0px';
			//this.G_d.style.padding='0px';
			this.G_d.style.padding='5px 12px';//改动
			this.G_d.style.borderWidth='1px';//改动
			this.G_d.style.borderStyle='solid';//改动
			this.G_d.style.borderColor='#95B8E7';//改动
			this.G_d.style.backgroundColor='#999999';
			//#E0ECFF
			this.iecss='';
			...
			if(typeof(ob.css)=='string'){
				...
				this.G_d.style.width='auto';
				this.G_d.style.height='auto';
				//this.G_d.style.backgroundColor='transparent';
				this.G_d.style.background='#E0ECFF';//改动
				this.G_d.className=ob.css;
				if(ob.css=='ui_confirm'){			
					this.G_d.innerHTML='<div class="panel-header panel-header-noborder window-header" style="width: 238.4px;background:#E0ECFF;"><div class="panel-title">提示</div><div class="panel-tool"><a class="panel-tool-close"></a></div></div>'+
					'<div class="easyui-window panel-body panel-body-noborder window-body" style="text-align:center;width: 238.4px;height:115.4px;line-height:114.4px;font-size:14px;border:1px solid #95B8E7;border-top:0;">'+ob.s
					+'</div><p class="ui_p"><input type="button" class="ui_button" value="确认" /><input type="button" class="ui_button ui_button_r" value="取消" /></p>'//改动
					...
					var ele=this.G_d.children[0].children[1].children[0];//改动
					ele.G_d=this.G_d;
					ele.yc=this.yc;
					ele.f=ob.f;
		...
		this.ct=function(wh){
			...
			if(wh[0]){
				this.G_d.children[1].style.width=wh[0]+'px';
				this.G_d.children[1].style.overflowX='hidden'
			};
			if(wh[1]){
				this.G_d.children[1].style.height=wh[1]+'px';
				this.G_d.children[1].style.overflowX='hidden';
				this.G_d.children[1].style.overflowY='auto'
			}
			else if(h-20<this.G_d.offsetHeight){
				this.G_d.children[1].style.height=h-20-39+'px';
				this.G_d.children[1].style.overflowX='hidden';
				this.G_d.children[1].style.overflowY='auto'
			};
		...
		this.xs=function(i){
			var T=this;
			this.G_d.style.filter='alpha(opacity='+(i*100)+')'+this.iecss;
			this.G_d.style.opacity=i;
