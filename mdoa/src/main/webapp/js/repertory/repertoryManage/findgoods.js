$(function(){
	...
	$(window).resize(function() {
		$('#findgoods').css('height',$('#loadarea').height()-31+'px');
		$('#findgoods .listForm').css('width',$('#loadarea').width()-502+'px');
		$('#findgoods .listForm .datagrid').css('width',$('#loadarea').width()-502+'px');
	});
	/*----表格化----*/
	$('#findgoodsdg').datagrid({
		...
		columns:[[
			...
			{field:'latestUnitPrice',title:'最新价格(元)',width:80,align:'center',},
			{field:'movingAverPrice',title:'移动平均价(元)',width:90,align:'center',},
	...
	$('#findgoods .query').click(function(){
		...
		var gooosSize = $.trim($('#findgoods .goodsSize').val());
	...
	$('#findgoods .newInsert .confirm').click(function(){
		...
		var provider = $('#findgoods .newInsert .popuparea .supplier option:selected').val();
...
$.ajax({
	...
	success:function(data){
		...
		$('#findgoods .update3 .getDepartmentName').append(str);
	},
	error:function(err){
		windowControl(err.status);
	}
...
function addsumBar(id,data){
	...
	str +='<td style="width:100px">总数量：</td>';
	str +='<td style="width:100px">'+data.totalNumber+'</td>';
	str += '</tr></table>';
