$(function(){
	...
	$('#goodspriceFormdg').datagrid({
		...
		columns:[[
			...
			{field:'latestUnitPrice',title:'最新价格',width:80,align:'center',},
			{field:'updateTime',title:'更新日期',width:140,align:'center',},
	...
	$('#goodspriceForm .query').click(function(){
		...
		var gooosSize = $.trim($('#goodspriceForm .goodsSize').val());
		...
		var repertoryName = $.trim($('#goodspriceForm .repertoryName option:selected').val());
		//var goodsLack = $('#goodspriceForm .repertoryName option:selected').val();
		var dataInfo;
		if(repertoryName == ""){
			dataInfo = {
				goodsName:goodsName,
...
function lookGoodsPriceForm(id,name){
	...
	$.ajax({
		...
		url:"../../reportForms/getPriceTrendData?getMs="+getMS(),
		success:function(data){
