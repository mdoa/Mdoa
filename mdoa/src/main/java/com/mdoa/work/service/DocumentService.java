...
public class DocumentService {
	...
	public List<DTreeModel> deptTree(String pid) {
		...
		setDocumentPicture(dTreeModels);
		return dTreeModels;
	}
	/**
	 * 设置图片
	 * @param list
	 */
	private void setDocumentPicture(List<DTreeModel> list) {
		for (int i = 0; i < list.size(); i++) {
			if (list.get(i).getAttributes().equals("1")) {
				list.get(i).setIconCls("icon-file");
			}else{
				list.get(i).setIconCls("icon-directory");
			}
			if (list.get(i).getChildren() != null || list.get(i).getChildren().size() != 0) 
				setDocumentPicture(list.get(i).getChildren());
		}
	}
	 ...
	 * @param workOfficeDocForm
	 */
	public void updateText(WorkOfficeDocForm workOfficeDocForm) {
		workOfficeDocForm.setCatalogId(StringUtil.getIdFromUrl(workOfficeDocForm.getUrl()));
		if (!documentDao.updateText(workOfficeDocForm)) 
	 ...
	 * @return
	 */
	public List<WorkOfficeDocForm> selectDocumentText(WorkOfficeDocForm workOfficeDocForm) {
		workOfficeDocForm.setCatalogId(StringUtil.getIdFromUrl(workOfficeDocForm.getUrl()));
		
		return documentDao.selectDocumentText(workOfficeDocForm);
	}
	/**
	 * 发布公文
	 * @param workOfficeDocForm
	 */
	public void publishOffice(WorkOfficeDocForm workOfficeDocForm) {
		workOfficeDocForm.setSuperCatalogId(StringUtil.getParentIdFromUrl(workOfficeDocForm.getUrl()));
		if (!documentDao.publishOffice(workOfficeDocForm)) 
			throw new RuntimeException("发布公文失败!");
	}
