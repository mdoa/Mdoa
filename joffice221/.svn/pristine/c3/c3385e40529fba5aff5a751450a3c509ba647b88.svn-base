	...
	public String decide(OpenExecution execution) {
		...
		if(list.size()>0){
			...
			try {
				...
				// In case to get parent variables when it is necessary
				if(vars.isEmpty() && execution instanceof ExecutionImpl) {
					ExecutionImpl executionImpl = (ExecutionImpl)execution;
					if(executionImpl.getSuperProcessExecution() != null) {
						vars = executionImpl.getSuperProcessExecution().getVariables();
					}
				}
				...
				it.eval(proHandleComp.getExeCode()==null?"":proHandleComp.getExeCode());
