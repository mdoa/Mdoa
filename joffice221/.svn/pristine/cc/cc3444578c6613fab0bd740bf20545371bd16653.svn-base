...
<table class="newsList" cellpadding="0" cellspacing="0" rules="rows">
	<c:forEach var="diary" items="${diaryList}">
		<tr>
			...
			<td><a href="#"
				onclick="eval(new DiaryDetail({diaryId:${diary.diaryId}}).show())"><c:choose><c:when test="${fn:length(diary.content)>30}">${fn:substring(diary.content,0,30)}...</c:when><c:otherwise>${diary.content}</c:otherwise></c:choose></a></td>
