...
<table class="newsList" cellpadding="0" cellspacing="0" rules="rows">
	<c:forEach var="message" items="${messageList}">
		<tr onclick="eval(new MessageDetail({receiveId:${message.receiveId}}).show())" style="cursor:pointer">
			...
			<td><a href="#"
				><c:choose><c:when test="${fn:length(message.shortMessage.content)>20}">${fn:substring(message.shortMessage.content,0,20)}...</c:when><c:otherwise>${message.shortMessage.content}</c:otherwise></c:choose></a></td>
