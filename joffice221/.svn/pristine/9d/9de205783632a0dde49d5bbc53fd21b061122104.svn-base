...
(function() {
	...
	var btnCmds = ['undo', 'redo', 'formatmatch', 'bold', 'italic',
			'underline', 'touppercase', 'tolowercase', 'strikethrough',
			'subscript', 'superscript', 'source', 'indent', 'outdent',
			'blockquote', 'pasteplain', 'pagebreak', 'selectall', 'print',
			'preview', 'horizontal', 'removeformat', 'time', 'date', 'unlink',
			...
			'selectinput', 'dictionary', 'userselector', 'roleselector',
	...
	var dialogBtns = {
		...
		ok : ['attachment', 'anchor', 'link', 'insertimage', 'map', 'gmap',
				...
				'insertframe', 'edittd', 'scrawl', 'template', 'background','importform']
	...
	editorui.snapscreen = function(editor, iframeUrl, title) {
		...
		editor.addListener('selectionchange', function() {
					ui
							.setDisabled(editor.queryCommandState('snapscreen') == -1);
				});
		return ui;
	};
	...
	editorui.emotion = function(editor, iframeUrl) {
		var ui = new editorui.MultiMenuPop({
					title : editor.options.labelMap['emotion']
							|| editor.getLang("labelMap.emotion") || '',
					editor : editor,
					className : 'edui-for-emotion',
					iframeUrl : editor.ui.mapUrl(iframeUrl
							|| (editor.options.iframeUrlMap || {})['emotion']
	 ...
	 * 添加按钮
	 */
	editorui.input = function(editor, iframeUrl, title) {
		iframeUrl = iframeUrl || (editor.options.iframeUrlMap || {})['input']
				|| iframeUrlMap['input'];
		title = title || editor.options.labelMap['input'] || '';
		if (!iframeUrl) {
			return;
		}
		var dialog = new editorui.Dialog({
		...
		dialog.render();
		editor.ui._dialogs['addinputbutton'] = dialog;
		...
		var ui = new editorui.Button({
					...
					title : title || editor.options.labelMap['input'] || '',
					onclick : function() {
						editor.execCommand("addinput");
					}
				});
		editor.addListener('selectionchange', function(t, v) {
					ui.setDisabled(editor.queryCommandState('addinput') == -1);
				});
		return ui;
	};
