...
//				})));
		if(this.is_history){
			...
			var dialog = this;
			conf.dialog = dialog;
			tb.x_buttons.add = tb.addItem({
				text : this.i18n.history_btn_text,
				tooltip : this.i18n.history_btn_tip,
				iconCls : "ext-ux-uploaddialog-addbtn",
				handler : function(){
					App.createUploadDialog(conf).show();
//					dialog.close();
				}
			});
		}
...
//					.child(tb.x_buttons.upload.buttonSelector).dom[tb.x_buttons.upload.tooltipType] = this.i18n.upload_btn_start_tip;
	...
	onCloseButtonClick : function() {
		...
		if(this.callback!=null){
			if(fileArr==''){
				Ext.ux.Toast.msg('操作信息', '当前上传个数文件为0,请添加或上传文件!');
				return;
			}else{
				var scope=this.scope?this.scope:this;
				this.callback.call(scope,fileArr);
			}
		}
		this.close();
