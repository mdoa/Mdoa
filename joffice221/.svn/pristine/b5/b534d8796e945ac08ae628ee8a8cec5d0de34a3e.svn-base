...
<%@ page
	import="com.htsoft.oa.model.admin.Conference,com.htsoft.oa.model.admin.ConfPrivilege"%>
...
<%
	...
	String updaters = "";
	String summarys = "";
	Set<ConfPrivilege> set = conference.getConfPrivilege();
...
<%@page import="com.htsoft.core.util.AppUtil"%><table class="table-info"
	...
	<tr style="height: 40px;">
		...
		<td colspan="3">
			<c:if test="${empty conference.checkReason }">无</c:if>
			<c:if test="${!empty conference.checkReason }">${conference.checkReason}</c:if>
		</td>
	</tr>
	<tr style="height: 25px;">
	...
	<tr style="height: 25px;">
		...
		<td colspan="3" style="text-align: inherit;">
			<c:if test="${empty conference.attachFiles}">无</c:if>
			<c:forEach varStatus="0" begin="0" items="${conference.attachFiles}" var="attachFile">
				<img src="<%=request.getContextPath()%>/images/system/download.png" />
				<a href="<%=request.getContextPath()%>/file-download?fileId=${attachFile.fileId}"
