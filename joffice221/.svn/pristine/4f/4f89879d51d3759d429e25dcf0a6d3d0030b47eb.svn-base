		...
		items : [{
					...
					items : [{
						...
						items : [{
									...
									handler : function(value) {
										if (value.getValue() == true) {
											Ext.getCmp('IssueScopeContainer')
													.hide();
											Ext.getCmp('AttendContainer')
													.hide();
											Ext.getCmp('PrincipalContainer')
													.hide();
										} else {
											Ext.getCmp('IssueScopeContainer')
													.show();
						...
						items : [{
									...
									scope : this,
									handler : function() {
										var participantsIds = Ext
												.getCmp('participantsIds');
										var participants = Ext
												.getCmp('participants');
										new UserDialog({
													single : false,
													scope : this,
	...
	if (this.planId != null && this.planId != 'undefined') {
		formPanel.getForm().load({
			...
			success : function(form, action) {
				...
				var startTime = getDateFromFormat(result.startTime,
						'yyyy-MM-dd HH:mm:ss');
				var endTime = getDateFromFormat(result.endTime,
						'yyyy-MM-dd HH:mm:ss');
			...
			handler : function() {
				...
				if (dep != null && dep != 'undefined' && dep != '') {
					if (dep.getValue() == true) {
						...
						var participants = Ext.getCmp('participants')
								.getValue();// 参与人
