...
OrgSettingView=Ext.extend(Ext.Panel,{
	...
	initUI:function(){
		...
		//组织树Panel
		this.orgPanel=new htsoft.ux.TreePanelEditor({
			treeType:'org',
			title:'组织',
		...
		//岗位树Panel
		this.posPanel=new htsoft.ux.TreePanelEditor({
			treeType:'pos',
			id:'posTree',
			title:'岗位',
...
//				break;
	...
	addCompany:function(orgId,orgName){
		var selObj = this.orgPanel.selectedNode;
		var orgDem=selObj.attributes.orgDem;
		new CompanyWin({
			orgSupId:orgId,
			supOrgName:orgName,
			orgDem:orgDem,
	...
	addOrg:function(){
		...
		var selObj = this.orgPanel.selectedNode;
		var orgId=selObj.id;
		var orgType=selObj.attributes.orgType;
		var orgDem=selObj.attributes.orgDem;
	...
	delOrg:function(){
		var orgId=this.orgPanel.selectedNode.id;
		if(orgId==0||orgId==1){
			Ext.ux.Toast.msg('操作信息','根节点组织不能删除,只能修改信息!');
			return;
		} 
			...
			if (btn == 'yes') {
				Ext.Ajax.request({
					...
					success:function(response,options){
						var result = Ext.util.JSON.decode(response.responseText);
						if(result.success){
							Ext.ux.Toast.msg('操作信息','成功删除组织!');	
							 this.orgPanel.root.reload();
						}			
						else
							Ext.ux.Toast.msg('操作信息','删除组织前请先删除该组织及其子组织下的用户!');		
					},
					failure : function(response,options) {
						Ext.ux.Toast.msg('操作信息','操作出错，请联系管理员！');
					}
