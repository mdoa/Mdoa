...
<%@ page
	import="com.htsoft.oa.model.admin.ConfSummary,com.htsoft.oa.service.admin.ConfSummaryService"%>
<!-- 
...
<table class="table-info" cellpadding="0" cellspacing="1" width="98%"
	...
	<tr>
		...
		<td style="width: 60%;">
			<c:if test="${confSummary.status==1}">发送</c:if> 
			<c:if test="${confSummary.status==0}">未发送</c:if>
		</td>
	</tr>
	<tr>
		<th style="width: 40%;">附件文件</th>
		<td style="width: 60%;">
			<c:if test="${empty confSummary.attachFiles}">无</c:if>
			<c:forEach items="${confSummary.attachFiles}" var="attachFile">
