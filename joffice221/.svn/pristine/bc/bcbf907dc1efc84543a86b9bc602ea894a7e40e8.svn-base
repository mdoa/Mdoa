...
PersonalTipsView = Ext.extend(Ext.Panel, {
	...
	initUI : function() {
		this.store = new Ext.data.JsonStore({
					...
					remoteSort : true,
					fields : ['tipsId', 'tipsName', 'content', {
								name : 'disheight',
								type : 'int'
							}, {
								name : 'diswidth',
					...
					items : ['<font color="red">提示：在空白面板双击输入！</font>', '->', {
								...
								handler : function() {
									if (this.store.getCount() > 0) {
										PersonalTipsView.prototype.deleteTips('all');
									} else {
...
//					 }
	...
	bodyContextClick : function(e, t, o) {
		if (!e.ctrlKey && !e.shiftKey) {
			...
			if (item == null) {
				...
				inputDiv
						.setStyle('left', e.getPageX() - myDiv.getLeft() + 'px');
							...
							if (item == null) {
								...
								if (this.store.getCount() > 0) {
									for (var i = 0; i < this.store.getCount(); i++) {
										var id = this.store.getAt(i)
												.get('tipsName');
										this.resizable(id);
									}
								}
								this.store
										.fireEvent('recordchange', this.store);
							}
						}, this);
				edit.startEdit(t, '');
	...
	saveRecord : function(store) {
		...
		for (var i = 0; i < store.getCount(); i++) {
			...
			if (this.curTipsId!=undefined||(record.dirty || tipsId == -1 || this.curTipsId == tipsId)) { // 得到所有修改过的数据
...
//		record.set('diswidth',width);
//		record.set('disheight',height);
		this.curWidth = width;
		this.curHeight = height;
		this.curTipsId = record.data.tipsId;
	...
	clickView : function(dataview, index, node, e) {
		...
		this.curWidth = record.data.diswidth;
		this.curHeight = record.data.disheight;
		this.curTipsId = record.data.tipsId;
...
//			var tempRecord = this.curRecord;
//			var level = this.curLevel;
//			tempRecord.set('dislevel', level);
//			this.curLevel = null;
//			this.curRecord = null;
//		}
		var myDiv = Ext.get("basicDiv");
		...
		if (el) {
			...
			if ((left != releft) && (top != retop)) {
				record.set('disleft', left);
				record.set('distop', top);
				this.store.fireEvent('recordchange', this.store);
			}
		}
	},
	// 删除便签
	deleteTips : function(id, name) {
					...
					if (btn == 'yes') {
						Ext.Ajax.request({
									...
									success : function(response, options) {
										var msg = (id=='all'?'所有':'');
										Ext.ux.Toast.msg('操作信息', '成功删除'+msg+'便签！');
										if (id != 'all') {
											var dataview = Ext
													.getCmp('dataView');
										...
										Ext.getCmp('PersonalTipsView').store
												.reload({
															params : {
																start : 0,
																limit : 1000
