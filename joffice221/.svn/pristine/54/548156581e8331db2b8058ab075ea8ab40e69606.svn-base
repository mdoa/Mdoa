Ext.ns('SuggestBoxLinkView');

/**
 * @author:
 * @class SuggestBoxLinkView
 * @extends Ext.Panel
 * @description [SuggestBoxLinkView]管理
 * @company 杭州梦德软件有限公司
 * @createtime:2010-01-16
 */
SuggestBoxLinkView = Ext.extend(Ext.Panel, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		SuggestBoxLinkView.superclass.constructor.call(this, {
					id : 'SuggestBoxLinkView',
					title : '意见箱管理',
			...
			columns : [{
						...
						dataIndex : 'senderIp'
								...
								actions : [{
											...
											style : 'margin:0 2px 0 2px',
											fn : function(record) {
												if (!this.isOutSide&&record.data.status == 0)
													return true;
												return false;
											}
										}, {
											...
											style : 'margin:0 2px 0 2px',
											fn : function(record) {
												if (!this.isOutSide && record.data.status == 1)
													return true;
												return false;
											}
										}, {
