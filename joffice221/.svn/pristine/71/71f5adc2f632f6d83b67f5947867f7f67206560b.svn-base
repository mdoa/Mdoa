        ...
        initEditorUI:function () {
            ...
            var popup = new baidu.editor.ui.Popup( {
                ...
                _onInputDelButtonClick:function(){
                	if(confirm(editor.getLang("confirmDelete"))){
                	    this.hide();
	                	if(popup.anchorEl){
                ...
                _onInputEditButtonClick:function(){
                	...
                	if(child){
                		...
                		if(external){
                			...
                			editor.execCommand(popup.anchorEl.className,false);
                			return;
                		}
                		switch(child.tagName.toLowerCase())
                		{
	                		case "a":
	            	    		...
	            	    		editor.ui._dialogs.addinputbutton.open();
	                			break;
	                		case "textarea":
	                			editor.curOpinion=popup.anchorEl;
	            	    		editor.ui._dialogs.addopinion.open();
	                			break;
	                		}
                	}
            ...
            if ( editor.options.imagePopup ) {
                editor.addListener( 'mouseover', function ( t, evt ) { //鼠标移入的右键属性
		    		...
		    		if(ifEl||ifElParent){//属性 name 含有自定义的 editable-input
		    			//是否为按钮
		    			var ifInput = el.className.indexOf("extend")>-1||el.parentElement.className.indexOf("extend")>-1;
		    			var delButton = ' <span onclick="$$._onInputDelButtonClick()" class="edui-clickable">'+editor.getLang("delete")+'</span>&nbsp;&nbsp;';
		    			...
		    			var html = popup.formatHtml(
			                       '<nobr>'+editor.getLang("property")+':'+delButton+ '</nobr>');
		    			if(editor.options.editorModel){
		    				if(editor.options.editorModel=="designModel"){
		    					html = popup.formatHtml(
		 		                       '<nobr>'+editor.getLang("property")+':'+ delButton +	                       
					   ...
					   if(ifInput){
						   html = popup.formatHtml(
		                       '<nobr>'+editor.getLang("property")+':'+ delButton+	                       
		                       '<span onclick="$$._onInputEditButtonClick(event, this);" class="edui-clickable">'+editor.getLang("modify")+'</span>&nbsp;&nbsp;'+
		                       '<span onclick="$$._onInputDialogButtonClick(event, this);" class="edui-clickable">'+editor.getLang("customDialog")+'</span></nobr>' );
					   }
