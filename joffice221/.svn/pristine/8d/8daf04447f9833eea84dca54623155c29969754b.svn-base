 ...
 * @data: 2010-1-10AM
 */
RelativeJobView = Ext.extend(Ext.Window, {
			// 构造函数
			...
			addRelativeUser : function(type,userId){
				...
				if(type == 1)
					...
					typeMsg = '下级';
				//判断是否选中相对岗位
				var node = Ext.getCmp('RelativeJobViewTreePanel').getSelectionModel().getSelectedNode();
				if(node != null && node.id > 0){
					// 获取已选用户userId
					var rStore = Ext.getCmp('RelativeJobView').gridPanel.getStore();
