...
<c:choose>
	<c:when test="${not empty taskId}">
		...
		<textarea id="subButtonRightstask_${taskId}" cols="120" rows="12" style="display:none">${subButtonRights}</textarea>
	...
	<c:otherwise>
		...
		<textarea id="subButtonRightsdef_${defId}" cols="120" rows="12" style="display:none">${subButtonRights}</textarea>
