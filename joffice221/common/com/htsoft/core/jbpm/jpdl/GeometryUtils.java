  ...
  public static Point getRectangleLineCrossPoint(Rectangle rectangle, Point p1, int grow) {   
    ...
    if (Math.abs(slopeLine) > slope - 1e-2) {   
      if (p1.y < rect.y) {   
        ...
        return new Point((int) Math.round((rect.y + rect.height - yIntercep) / slopeLine), rect.y + rect.height);   
      }   
    }   
    if (p1.x < rect.x) {   
      return new Point(rect.x, (int) Math.round(slopeLine * rect.x + yIntercep));   
    } else {   
      return new Point(rect.x + rect.width, (int) Math.round(slopeLine * (rect.x + rect.width) + yIntercep));   
    }   
  }   
}  
