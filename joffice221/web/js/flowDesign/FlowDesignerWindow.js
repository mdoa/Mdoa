...
FlowDesignerWindow = Ext.extend(Ext.Window, {
	constructor : function(config) {
		...
		//		
		// designer.setAttribute('code','com.htsoft.jbpm.designer.ProcessApplet.class');
		...
		this.formPanel = new Ext.FormPanel({
					...
					items : [{
								...
								allowBlank : false,
								url : __ctxPath
										+ '/system/flowTreeGlobalType.do?catKey=FLOW'
							}, {
								text : '流程的名称',
								xtype : 'label'
							}, {
		...
		FlowDesignerWindow.superclass.constructor.call(this, {
					...
					maximizable : true,
					closeAction : 'close',
					items : [this.leftPanel, this.formPanel],
					buttonAlign : 'center',
					buttons : [{
								text : '保存',
								scope : this,
	...
	onSave : function(deploy) {
		...
		document.getElementById("drawDefXml").value = document.applets.flowDesigner
				.getData();
		...
		if (this.formPanel.getForm().isValid()) {
			this.formPanel.getForm().submit({
						...
						success : function(fp, action) {
							var proDefinitionGrid = Ext
									.getCmp("ProDefinitionGrid");
							if (proDefinitionGrid != null) {
								proDefinitionGrid.getStore().reload();
							}
							var proDefinitionGrid0 = Ext
									.getCmp("ProDefinitionGrid0");
