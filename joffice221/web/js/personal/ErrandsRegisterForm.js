 ...
 * 请假单详细信息
 * 
 * @class ErrandsRegisterForm
 * @extends Ext.Window
 */
ErrandsRegisterForm = Ext.extend(Ext.Window, {
	// 构造函数
	constructor : function(_cfg) {
		...
		// 调用父类构造
		ErrandsRegisterForm.superclass.constructor.call(this, {
			...
			items : [{
						...
						listeners : {
							...
							select : function(combo, record, index) {// 选择审批人赋值
								this.formPanel
										.getCmpByName('errandsRegister.approvalName')
	...
	save : function() {
		...
		if (formPanel.getForm().isValid()) {
			// 验证外出开始时间大于结束进间
			var st = formPanel.getCmpByName('errandsRegister.startTime')
					.getValue();
			var et = formPanel.getCmpByName('errandsRegister.endTime')
					.getValue();
			var sd = Date.parse(st);
			var ed = Date.parse(et);
			if (sd > ed) {
				Ext.ux.Toast.msg('操作信息', '请假开始时间大于结束时间,不能保存!');
				return;
			};
			// 提交
			$postForm({
