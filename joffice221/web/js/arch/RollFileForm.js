...
RollFileForm = Ext.extend(Ext.Window, {
		...
		this.formPanel = new Ext.FormPanel({
			...
			items : [{
						...
						items:[{
								width:100,
								fieldLabel : '文件分类',
								hiddenId:'rollFileProTypeId',//隐藏域的id
						 		hiddenName: 	'rollFile.globalType.typeName',
						 		name : 'rollFile.globalType.typeName',
						 		xtype:'combotree',	
						 		url: __ctxPath + '/system/treeGlobalType.do?catKey=AR_RLF',// 不把根目录显示出来
						 		listeners : {
									scope : this,
									'select' : function(combo, record, index) {									
										this.formPanel.getForm().findField('rollFile.proTypeId')
												.setValue(record.id);
									}
								}
						}]
					...
					},				
					{
						...
						items : {
							...
							listeners : {
								scope : this,
								'select' : function(combo, record, index) {
									this.formPanel.getForm().findField('rollFile.archRoll.rollNo')
											.getStore().load({
												params : {
	 ...
	 */
	save : function(type) {
		var rollNo=	this.formPanel.getCmpByName('rollFile.archRoll.rollNo').getValue();
		if(rollNo.length<1){
			Ext.ux.Toast.msg("操作信息", "如果修改全宗号，还需重新选择案卷号；如果案卷下拉框没有内容,就没有属于该全宗号下的案卷号，请先在【案卷管理】添加属于该全宗号下的案卷信息!");
			return;
		}
		if (this.rollFileId == null || this.rollFileId == 'undefined') {
		...
		if (this.formPanel.getForm().isValid()) {
			...
			this.formPanel.getForm().submit({
				...
				success : function(form, action) {
					this.rollFileId = action.result.rollFileId;
					this.load();
