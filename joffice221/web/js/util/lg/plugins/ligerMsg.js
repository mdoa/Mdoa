(function($) {
	$.ligerMsg = {
		...
		_open : function(type, msg, bottom, buttons) {
			...
			$(topWindow.document).find(this._boxId).remove();
			...
			var objMsgbox=$(this._boxId,$(topWindow.document));
			...
			//从右下角冒出来
				...
				var height = $(topWindow.document.body).height();
		...
		close : function() {
			...
			if (objMsgbox.length > 0) {
				//在顶部
				if (objMsgbox.position().top <= 0) {
					objMsgbox.animate({top : -objMsgbox.height()}, 500, function() {
						$(this).remove();
					});
				}
				else {
					var height = $(topWindow.document.body).height();
					objMsgbox.animate({top : height}, 500, function() {
						$(this).remove();
					});
				}
			}
