...
/**
 * jquery自定义表单验证插件 使用方法： 在需要做验证的输入框，单选框，多选框，下拉框中加入validate属性 validate:写法如下：
 * {required:true,email:true,maxLength:50} 如： <input type="text" name="username"
 * value="" validate="{required:true,maxlength:50}"/> 注意一组单选框，或多选框
 * 只需在其中一个input标记中 加入validate 属性 如： <input type="checkbox" name="a" value="1"
 * validate="{required:true}" tipId="errorA"/> <input type="checkbox" name="a"
 * value="2" /> <input type="checkbox" name="a" value="3" />
 * 
 * tipId:错误信息显示的容器ID,设置了这个属性后，错误信息会显示到该标签中。 <input type="text" name="name"
 * validate="{reqired:true}" tipId="errorA"/><label id="errorA"></label>
 * 
 * 
 * 调用方式:
 * 
 * $("a.save").click(function(){ var rtn=$("#shipOrderForm").form().valid();
 * if(rtn){ $("#shipOrderForm").submit(); } }); 同时也可以扩展验证的规则 var
 * rtn=$("#form").form({ //扩展验证规则 追加到已有的规则中 rules:[{ //规则名称 name:"QQ", //判断方法 返回
 * true 或false rule:function(v){ }, //错误的提示信息 msg:""
 * 
 * }], //显示的错误信息样式 element 当前验证的元素，msg：错误信息
 * errorPlacement:function(element,msg){ }, //成功后的样式 element 当前验证的元素
 * success:function(element){ }}).valid()
 */
(function($) {
	$.extend($.fn, {
		...
		rules : [{
					...
					name : "field",
					rule : function(v) {
						return /^[A-Za-z]{1}([A-Za-z0-9\_]+)?$/.test(v.trim());
					},
