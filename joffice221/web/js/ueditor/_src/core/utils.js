...
/** @lends baidu.editor.utils.prototype */
{
	 ...
	 *            delay 延迟时间，单位为毫秒
	 * @param {Boolean}
	 *            exclusion 是否互斥执行，true则执行下一次defer时会先把前一次的延迟函数删除
	...
	findNode : function(nodes, tagNames, fn) {
		for (var i = 0, ci; ci = nodes[i++];) {
			if (fn
					? fn(ci)
					: this.indexOf(tagNames, ci.tagName.toLowerCase()) != -1) {
				return ci;
			}
		}
	},
	...
	cssStyleToDomStyle : function() {
		...
		return function(cssName) {
			return cache[cssName]
					|| (cache[cssName] = cssName.toLowerCase().replace(/-./g,
							function(match) {
		...
		return function(doc, obj, fun) {
			...
			if (item) {
				if (utils.isArray(item.funs)) {
					item.ready ? fun() : tmpList[obj.src || obj.href].funs
							.push(fun);
				}
				return;
			}
			tmpList[obj.src || obj.href] = fun ? {
				'funs' : [fun]
			} : 1;
	 ...
	 * @param {String}
	 *            str 字符串
	 * @return {String} str 解码单引号和双引号后的字符串
	 */
