...
//						Ext.MessageBox.show({
//									title : '操作信息',
//									msg : '“印章”插件必需使用ie浏览器进行浏览',
//									buttons : Ext.MessageBox.OK,
//									icon : 'ext-mb-warning',
//									scope : this,
//									fn : function() {
//										Ext.getCmp('MakeSealForm').close();
//									}
//								});
//					}, 500);
//		}
		Ext.useShims = true;
		this.ekeyPanel = new NtkoSignPanel({
					height : 200,
					border : true
				});
	...
	saveSign : function() {
		if (this.formPanel.getForm().isValid()) {
			...
			var path = '';
			var fileName = this.formPanel.getCmpByName('seal.sealName')
					.getValue();
			var belongName = this.formPanel.getCmpByName('seal.belongName')
					.getValue();
			var password = this.formPanel.getCmpByName('sealPassword')
					.getValue();
	...
	save : function(fileId, fileName, belongName, password) {
		var result = this.ekeyPanel
				.save(fileId, fileName, belongName, password);
	...
	setValue : function(ekeyObj) {
		...
		this.formPanel.getCmpByName('seal.belongName')
				.setValue(ekeyObj.SignUser);
