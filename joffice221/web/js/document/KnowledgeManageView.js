...
KnowledgeManageView = Ext.extend(Ext.Panel, {
	...
	initUI : function() {
		...
		this.tpl = new Ext.XTemplate(
				'<ul>',
				'<tpl for=".">',
				'<li class="thumb-wrap" style="line-height:2em" id="{fileId}" name="{fileName}" type="{isFolder}" rightmod="{rightMod}" rightdel="{rightDel}" ',
				'ext:qtip="',
	...
	clickNode : function(node) {
		...
		if (folderId != null) {
			if (folderId == 0) {
				this.rightPanel.setTitle('所有知识列表');
			} else {
				this.rightPanel.setTitle('[' + node.text + ']' + '知识列表');
			}
		}
		store.reload({
					params : {
	...
	searchDisable : function() {
		...
		Ext.getCmp('UpFolderButton').disable();
		Ext.getCmp('TopFolderButton').disable();
		this.isSearching = true;
	},
	/**
	 * 查询状态-恢复操作
	...
	searchEnable : function() {
		...
		Ext.getCmp('UpFolderButton').enable();
		Ext.getCmp('TopFolderButton').enable();
		this.isSearching = false;
	},
	// =====改变路径操作========
	/**
	...
	dbclickNode : function(dataview, index, node, e) {
		...
		if (!Ext.isEmpty(node)) {
			...
			var menuItems = [{
						...
						text : '修改',
						iconCls : 'btn-edit',
						hidden : ((rightMod == 1 || isAll) && type == 0)
								? false
								: true,// 有权限并且类型是目录的 有修改
						scope : this,
