...
<%
		...
		if(opt != null && !"".equals(opt)){
			...
			if(isSend.equals("true")){
				filter.addFilter("Q_shortMessage.senderId_L_EQ",userId);//只取当前用户发送的信息
			}else filter.addFilter("Q_userId_L_EQ",userId);//只取当前用户收到的信息
		...
		String senderOrReciver = inMessage.getShortMessage().getSender();
		if(isSend!=null){
			if(isSend.equals("true")){
				str = "发送到：";
				senderOrReciver = inMessage.getUserFullname();
			}
		}
		request.setAttribute("isRead",isRead);
		request.setAttribute("inMessage",inMessage);
		request.setAttribute("str",str);
		request.setAttribute("senderOrReciver",senderOrReciver);
%>
