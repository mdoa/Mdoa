...
Ext.define('Ext.ux.TouchCalendarView', {
                            ...
                            '<tr>',
                                '<tpl for="this.getDaysArray(values)">',
                                    '<th class="{[this.getHeaderClass(xindex)]}">',
                                        '<tpl if="xindex === 4">',
                                            '<span>{[Ext.Date.format(this.me.currentDate, "Y")]}年  {[Ext.Date.format(this.me.currentDate, "F")]}</span>',
                                        '</tpl>',
                                        '{date:date("B")}',
                                    '</th>',
                                '</tpl>',
	...
	applyViewMode: function(viewMode){
		...
		this.setScrollable({
			direction: viewMode.toUpperCase() === 'DAY' ? 'vertical' : 'auto',
		});
						...
						'<table class="{[this.me.getViewMode().toLowerCase()]}">',
                            '<thead>',
                                '<tr>',
                                    ...
                                    '</th>',
                                    '<th>',
                                        '<span>{[Ext.Date.format(values[0].date, "Y年  F j日  B")]}</span>',
                                    '</th>',
