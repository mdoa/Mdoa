...
public class StandSalaryAction extends BaseAction {
	...
	public String get() {
		...
		jsonString = mapper.toDataJson(standSalary);
	...
	public String save() {
		...
		if (pass) {
			if (standSalary.getStandardId() != null) {
				// 修改旧纪录
				standSalary.setModifyName(ContextUtil.getCurrentUser()
						.getFullname());
				standSalary.setModifyTime(new Date());
			} else {
				standSalary.setSetdownTime(new Date());
				standSalary.setFramer(ContextUtil.getCurrentUser()
						.getFullname());
			}
			...
			if (StringUtils.isNotEmpty(deleteItemIds)) {
				...
				for (String id : ids) {
					if (StringUtils.isNotEmpty(id)) {
						if (standSalaryItemService.get(new Long(id)) != null)
							;
			...
			if (StringUtils.isNotEmpty(data)) {
				Gson gson = new Gson();
				StandSalaryItem[] standSalaryItems = gson.fromJson(data,
						StandSalaryItem[].class);
	...
	public String check() {
		...
		StandSalary checkStandard = standSalaryService.get(standSalary
				.getStandardId());
		...
		if (StringUtils.isNotEmpty(status)
				&& Short.valueOf(status) == STATUS_PASS) {
