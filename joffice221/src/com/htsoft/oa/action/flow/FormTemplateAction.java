...
public class FormTemplateAction extends BaseAction{
	...
	public String mappings(){
		...
		List<Node> nodes = jbpmService.getFormNodesByDeployId(formDefMapping.getDeployId());
		
		List<FormTemplate> templateList = new ArrayList<FormTemplate>();
		...
		if(templateList.size()>0){
			this.setJsonString(mapper.toResultJson(templateList));
		}	
		return SUCCESS;
	}
	/**
	...
	public String saveList(){
		...
		if (StringUtils.isNotEmpty(formTemps)) {
			...
			FormTemplate[] formTemplates = (FormTemplate[]) mapper.toObject(
					formTemps, FormTemplate[].class);
			//保存流程的表单设置
			formTemplateService.saveFormTemplates(formTemplates);	
		}
		return SUCCESS;
	}
