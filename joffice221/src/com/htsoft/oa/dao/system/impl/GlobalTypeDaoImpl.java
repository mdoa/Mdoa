...
public class GlobalTypeDaoImpl extends BaseDaoImpl<GlobalType> implements
	 ...
	 * 
	 * @see
	 * com.htsoft.ent.dao.system.GlobalTypeDao#getByParentIdCatKey(java.lang
	...
	public List<GlobalType> getByRightsCatKey(AppUser curUser, String catKey) {
		...
		StringBuffer hql = new StringBuffer(
				"select gt from ProDefRights pr right join pr.globalType gt  where gt.catKey = ? and (pr.userIds like ?  or pr.depIds like ? ");
		params.add(catKey);
		params.add(uIds);
		params.add(dIds);
	...
	public GlobalType findByFileType(String fileType) {
		...
		Log.debug("GlobalType : " + hql);
		List<GlobalType> list = findByHql(hql, new Object[] { fileType,
				"ATTACHFILE_TYPE" });
	...
	public List<GlobalType> getByCatKey(String catKey) {
		String hql = " from GlobalType gt where  gt.catKey = ? order by gt.sn asc";
	...
	public GlobalType getByDictNodeKey(String nodeKey) {
		String hql = " from GlobalType gt where gt.nodeKey = ? and gt.catKey = ? order by gt.sn asc";
		List<GlobalType> list = findByHql(hql, new Object[] { nodeKey, "DIC" });
		return  list != null && list.size() > 0 ? list.get(0) : null;
	}

	@Override
	public List<GlobalType> getByCatKeyAndGTsn(String catKey, Integer sn) {
		String hql = " from GlobalType gt where  gt.catKey = ? and gt.sn > ? order by gt.sn asc";
		List<GlobalType> list = findByHql(hql, new Object[] { catKey, sn });
		return list;
	}
}