package com.htsoft.oa.core.jbpm.servlet;
...
public class JpdlImageServlet extends HttpServlet {
	 ...
	 * 
	 */
	private static final long serialVersionUID = -7700449547429628188L;
	...
	private ProcessRunService processRunService = (ProcessRunService) AppUtil
	...
	public String getProcessDefintionXml(HttpServletRequest request) {
		...
		if (StringUtils.isNotEmpty(isSubFlow)) {
			...
			ProcessInstance pi = (ProcessInstance) taskImpl
					.getProcessInstance().getSuperProcessExecution();
		...
		if (StringUtils.isNotEmpty(runId)) {
			...
			if (processRun.getPiId() != null && processRun.getRunStatus().shortValue() == ProcessRun.RUN_STATUS_RUNNING.shortValue() ) {
				return jbpmService.getDefinitionXmlByPiId(processRun.getPiId());
			} else {
				return jbpmService.getDefinitionXmlByDefId(processRun
	...
	public void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			if ("true".equals(isSubFlow) && StringUtils.isNotEmpty(taskId)) {
				TaskImpl taskImpl = (TaskImpl) jbpmService.getTaskById(taskId);
				pi = (ProcessInstance) taskImpl.getProcessInstance()
			...
			if (pi != null) {
				Set<String> activeActivityNames = pi.findActiveActivityNames();
			...
			if (!"true".equals(genMap)) {
				...
				ImageIO.write(drawer.draw(jpdlModel), "png",
				...
				for (AnchorArea anchor : list) {
					writer.println("<area shape='rect' coords='"
							...
							+ anchor.getEndY()
							+ "'"
							+ " href='#' onclick='javascript:ProDefinitionSetting.clickNode("
							+ defId + ",\"" + anchor.getActivityName()
							+ "\",\"" + anchor.getNodeType() + "\");'>");
				}
			}
